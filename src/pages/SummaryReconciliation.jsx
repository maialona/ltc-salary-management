...
export default function SummaryReconciliation() {
  ...
  const loadPeriod = (p) => {
    ...
    setWarnings([]);
    if (cq && welfare) {
      setRows(reconcileSummaries(cq, welfare));
    } else {
      setRows([]);
    }
  };
  ...
  const handleFileUpload = async (e) => {
    ...
    setTimeout(async () => {
      try {
        ...
        const reconciledRows = reconcileSummaries(caseQuantity, welfareData);
                    ...
                    {filteredRows.length === 0 ? (
                      ...
                      return (
                        <tr key={idx} className="group transition-colors border-b hover:bg-white/[0.05]" style={{ borderColor: 'var(--glass-border)' }}>
                          ...
                          {/* Record side */}
                          {isMissR ? (
                            <td colSpan={6} className="px-4 py-3 text-center text-sm border-r" style={{ color: 'var(--text-secondary)', borderColor: 'var(--glass-border)', opacity: 0.45 }}>
                              — 無資料 —
                            </td>
                          ) : (
                            <>
