...
function dbToClient(row) {
  return {
    ...
    pensionFeeNote:         row.pension_fee_note          || '',
    bgsOtherDeductionNote:  row.bgs_other_deduction_note  || '',
    withholdingTaxNote:     row.withholding_tax_note      || '',
    otherDeductionNote:     row.other_deduction_note      || '',
  };
}
  ...
  const { rows } = await client.query(
    `INSERT INTO deductions (
     ...
     ON CONFLICT (institution_code, period, emp_id) DO UPDATE SET
       ...
       bgs_other_deduction_note = EXCLUDED.bgs_other_deduction_note,
       withholding_tax_note     = EXCLUDED.withholding_tax_note,
